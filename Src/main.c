...
/* USER CODE BEGIN 0 */
	...
	void Task2 (void *p){
		...
		for( ;; ){
            if(xSemaphoreTake(Gatekeeper, 1000)){                // Testa se a tarefa possui o semáforo
            	xQueueSend(Queue_RX, GetDataFromUSB, 100)        // Envia para a fila dos elementos recebidos, o dado recebido via USB, aguardando no máx. 100ms
            }
		}
	}
	...
	void Task3 (void *p){
		...
		for( ;; ){
			if(xSemaphoreTake(Gatekeeper, 1000)){                // Testa se a tarefa possui o semáforo
				...
				if()                                            // Testa condição para acender os lEDs
	...
	void Task4 (void *p){
		...
		for( ;; ){
			//Quando a interrupção ocorrer, envia dados da fila de transmissão para a porta USB
			if(IsButtonPressed(GPIOB_Pin)){                      // Checa se o butão foi apertado
				...
				ISR_Handle();                                    // Chama na rotina de interrupção para tratar do pressionamento do botão
			}
		}
	}
...
int main(void)
{
  ...
  static const uint8_t ucIPAddress[ 4 ] = { 192, 168, 137, 10 };
  static const uint8_t ucNetMask[ 4 ] = { 255, 255, 255, 0 };
  static const uint8_t ucGatewayAddress[ 4 ] = { 192, 168, 137, 1 };
  static const uint8_t ucDNSServerAddress[ 4 ] = { 192, 168, 137, 1};
